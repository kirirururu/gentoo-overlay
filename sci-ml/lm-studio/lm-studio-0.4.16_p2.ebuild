...
LMS_PV=${PV/_p/-}
SRC_URI="https://installers.lmstudio.ai/linux/x64/${LMS_PV}/LM-Studio-${LMS_PV}-x64.deb"
S="${WORKDIR}"
...
src_install() {
	if ! use cuda; then
		rm -r opt/LM-Studio/resources/app/.webpack/bin/extensions/backends/llama.cpp-linux-x86_64-nvidia-cuda-avx2-*
		rm -r opt/LM-Studio/resources/app/.webpack/bin/extensions/backends/vendor/linux-llama-cuda-vendor-v1
	fi

	if ! use vulkan; then
		rm -r opt/LM-Studio/resources/app/.webpack/bin/extensions/backends/llama.cpp-linux-x86_64-vulkan-avx2-*
	...
	lmlink_dir=$(find opt/LM-Studio/resources/app/.webpack/bin/extensions/frameworks -type d -name 'lmlink-connector-*' -maxdepth 1)
	fperms 755 /${lmlink_dir}/lmlink-connector
